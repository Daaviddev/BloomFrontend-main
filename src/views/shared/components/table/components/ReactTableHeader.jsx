...
const Header = ({ column, isSortable }) => (
    <span className="react-table__column-header">
        <span className={isSortable ? "react-table__column-header sortable" : ""}>{column.render("Header")}</span>
...
const Sorting = ({ column }) => (
    <span className="react-table__column-header sortable">
        {column.isSortedDesc === undefined ? <SortIcon /> : <span>{column.isSortedDesc ? <SortDescendingIcon /> : <SortAscendingIcon />}</span>}
    </span>
);
            ...
            {headerGroups.map(headerGroup => (
                <tr {...headerGroup.getHeaderGroupProps()} className="react-table thead tr">
                    {headerGroup.headers.map(column => (
                        <th {...column.getHeaderProps(headerPropsSortable)} {...column.getHeaderProps(headerPropsResize)} {...column.getHeaderProps()}>
                            <Header column={column} isSortable={isSortable} />
                            {isResizable && <div {...column.getResizerProps()} className={`resizer ${column.isResizing && "isResizing"}`} />}
                        </th>
                    ))}
                </tr>
            ))}

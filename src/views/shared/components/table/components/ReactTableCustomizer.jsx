    ...
    return (
        <Row className="react-table__customizer">
            <div className="table__collapse">
                ...
                {collapse && <button className="table__collapse-back" aria-label="table__collapse-back" type="button" onClick={handleOpen} />}
                <Collapse isOpen={collapse} className="table__collapse-content">
                    ...
                    <div className="table__collapse-item">
                        {arrayTableCustomizerSingleToggle.map(item => (
                            <ReactTableCustomizerToggle key={item.id} text={item.text} handleClick={item.func} isChecked={item.isChecked} />
                        ))}
                    </div>
                    {fullCustomizer && (
                        <div className="table__collapse-item">
                            {arrayTableCustomizerAddictionToggle.map(item => (
                                <div id={`tooltip-${item.id}`} key={item.id}>
                                    <ReactTableCustomizerToggle key={item.id} text={item.text} handleClick={item.func} isChecked={item.isChecked} isDisabled={item.isDisabled} />
                                    {item.isDisabled && (
                                        <UncontrolledTooltip className="table__collapse-item--tooltip" placement="left" target={`tooltip-${item.id}`}>
                                            {item.tooltip}
                                        </UncontrolledTooltip>
                                    )}
                                </div>

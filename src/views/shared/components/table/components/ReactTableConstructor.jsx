...
const ReactTableConstructor = ({ tableConfig, tableOptions, tableOptionalHook }) => {
    const { isEditable, isResizable, isSortable, withPagination, withSearchEngine, manualPageSize, placeholder } = tableConfig;
    const {
        getTableProps,
        getTableBodyProps,
        headerGroups,
    ...
    } = useTable(tableOptions, useGlobalFilter, useSortBy, usePagination, useResizeColumns, useRowSelect, ...tableOptionalHook);
    ...
    return (
        <div>
            ...
            <div className={withPagination ? "table react-table" : "table react-table table--not-pagination"}>
                <table {...getTableProps()} className={isEditable ? "react-table editable-table" : "react-table resizable-table"}>
                    ...
                    />
                    {(pageCount === pageIndex + 1 || (!withPagination && rows.length !== 0)) && <ReactTableFooter footerGroups={footerGroups} />}
                </table>
            </div>

    ...
    return (
        <Pagination className="pagination" dir="ltr">
            <div className="pagination">
                <PaginationLink className="pagination__link pagination__link--arrow" type="button" onClick={() => gotoPage(0)} disabled={!canPreviousPage}>
                    <ChevronDoubleLeftIcon className="pagination__link-icon" />
                </PaginationLink>
                <PaginationLink className="pagination__link pagination__link--arrow" type="button" onClick={previousPage} disabled={!canPreviousPage}>
                    <ChevronLeftIcon className="pagination__link-icon" />
                </PaginationLink>
                {arrayPageIndex.map(i => (
                    <PaginationItem className="pagination__item" active={pageIndex === i} key={i}>
                        <PaginationLink key={i} className="pagination__link" type="button" onClick={() => gotoPage(i)}>
                            {i + 1}
                        </PaginationLink>
                    </PaginationItem>
                ))}
                <PaginationItem className="pagination__item">
                    <PaginationLink className="pagination__link pagination__link--arrow" type="button" onClick={nextPage} disabled={!canNextPage}>
                        <ChevronRightIcon className="pagination__link-icon" />
                    </PaginationLink>
                </PaginationItem>
                <PaginationItem className="pagination__item">
                    <PaginationLink className="pagination__link pagination__link--arrow" type="button" onClick={() => gotoPage(pageOptions.length - 1)} disabled={!canNextPage}>

...
const ReactTableDnDBody = ({ page, getTableBodyProps, prepareRow, updateDraggableData, theme }) => {
    ...
    return (
        <DragDropContext onDragEnd={onDragEnd}>
            <Droppable droppableId="draggable" type="data">
                {provided => (
                    <tbody className="table table--bordered" ref={provided.innerRef} {...getTableBodyProps()} {...provided.droppableProps} style={getListStyle(theme)}>
                        {page.map(val => {
                            prepareRow(val);
                            return (
                                <Draggable draggableId={`${val.id}`} key={val.id} index={Number(val.id)} type="data">
                                    {provide => (
                                        <tr

...
const ChromeColorPickerField = ({ onChange, name }) => {
    ...
    const handleClick = e => {
        e.preventDefault();
        setChromeColorActive({ displayColorPicker: !displayColorPicker, active: !active });
    };
    ...
    return (
        <div className="color-picker">
            <button type="button" className={`color-picker__button${active ? " active" : ""}`} onClick={handleClick} id={name}>
                <p className="color-picker__color">{color}</p>
                <div className="color-picker__color-view" style={{ backgroundColor: color }} />
            </button>
            <Popover isOpen={displayColorPicker} target={name} placement="bottom" className="color-picker__popover">
